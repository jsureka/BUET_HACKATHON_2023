...
contract ArtworkMarketplace is ERC721URIStorage {
  ...
  address payable owner;
    ...
    enum DeliveryStatus { NotDelivered, Delivered, Received }
    ...
    modifier onlyCreator(uint256 artworkId, address creator) {
        require(creator == artworks[artworkId].creator, "Only the artwork creator can perform this action");
        _;
    }
    ...
    function updateArtworkQuantity(uint256 artworkId, uint256 quantity, address creator) external onlyCreator(artworkId , creator) artworkExists(artworkId) {
    ...
    function updateOrderDeliveryStatus(uint256 orderId, DeliveryStatus status) external orderExists(orderId) {
        orders[orderId].deliveryStatus = status;
        emit OrderDeliveryStatusUpdated(orderId, status);
    }

    // Place an order for an artwork
    function placeOrder(uint256 artworkId) public payable artworkExists(artworkId) {
        require(msg.value ==  artworks[artworkId].price / 10, "Incorrect payment amount"); 
        require(msg.sender != artworks[artworkId].creator , "Product creator cannot buy");       
        require(artworks[artworkId].quantity > 0, "Product out of stock");
        (bool sent,) = payable(artworks[artworkId].creator).call{value: msg.value}("");
        artworks[artworkId].quantity--;
        totalOrders++;
        orders[totalOrders] = Order(totalOrders, artworkId, msg.sender, DeliveryStatus.NotDelivered);
        emit OrderPlaced(totalOrders);
    }

    //Confirm receiving the order
    function confirmOrderReception(uint256 orderId) external payable orderExists(orderId) {
        Order storage order = orders[orderId];        
        require(msg.value == (artworks[ orders[orderId].artworkId].price * 9) / 10, "Incorrect payment amount");
        require(order.buyer == msg.sender, "Only the buyer can confirm order reception");
    ...
    function placeBid(uint256 artworkId, uint256 amount, uint256 currentTime) external payable artworkExists(artworkId) {
        require(amount > 0, "Bid amount must be greater than 0");
        require(artworks[artworkId].isPremium == true, "This product is not approved for bidding");
        require(currentTime < artworks[artworkId].deadline, "Time's Up");
        artworks[artworkId].totalBids++;
        artworks[artworkId].bids.push(Bid(artworks[artworkId].totalBids, artworkId, msg.sender, amount));
