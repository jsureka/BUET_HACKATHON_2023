...
export default function SingleArt({ artwork, imgSrc, Title, Artist, price, premium }) {
    ...
    const [artist, setArtist] = useState(Artist.toString());
    ...
    useEffect(() => {
        ...
        const artist = Artist.toString();
        const first = artist.substring(0, 3);
        const second = '...';
        const third = artist.substring(artist.length - 5, artist.length);

        const half = first.concat(second);
        setArtist(half.concat(third));
                ...
                <div className="bg-primary px-10 py-4">
                    ...
                    <div className="flex flex-row items-center justify-between">
                        ...
                        <div className="mt-4">
                            {premium ? (
                                <>
                                    <button className="rounded-lg bg-secondary-2 py-2 px-3 font-display hover:bg-secondary-1-dark" onClick={() => handleClickOpen()}>
                                        Place Bid
                                    </button>
                                    <BidModal isOpen={open} closeModal={handleClose} />
                                </>
                            ) : (
                                <button
